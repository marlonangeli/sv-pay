...
public class InactiveAccountTests(IntegrationTestWebAppFactory factory) : BaseIntegrationTest(factory), IAsyncLifetime
{
    ...
    [Fact]
    public async Task Should_InactiveAccount_When_AccountIsActive()
    {
        // Arrange
        var command = new InactiveAccountCommand(_activeAccountId);

        // Act
        var response = await HttpClient.PutAsJsonAsync("/api/v1/accounts/inactive", command);
    ...
    [Fact]
    public async Task Should_ActivateAccount_When_AccountIsInactive()
    {
        // Arrange
        var command = new InactiveAccountCommand(_inactiveAccountId, Active: true);
        ...
        var account = await DbContext.Accounts.FindAsync(_inactiveAccountId);
        account.Should().NotBeNull();
    ...
    {
        // Arrange
        var command = new InactiveAccountCommand(Guid.NewGuid());
    ...
    [Fact]
    public async Task Should_ReturnBadRequest_When_InactivatingBlockedAccount()
    {
        // Arrange
        var command = new InactiveAccountCommand(_blockedAccountId);
    ...
    [Fact]
    public async Task Should_ReturnBadRequest_When_InactivatingInactiveAccount()
    {
        // Arrange
        var command = new InactiveAccountCommand(_inactiveAccountId);
    ...
    [Fact]
    public async Task Should_ReturnBadRequest_When_ActivatingBlockedAccount()
    {
        // Arrange
        var command = new InactiveAccountCommand(_blockedAccountId, Active: true);
